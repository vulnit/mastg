...
Secure exception and error handling in Android is about preventing the leakage of sensitive information, managing failures gracefully, and ensuring that errors don't compromise security. User-facing error messages should remain generic, while controlled logging is reserved for developers. The [OWASP DevGuide](https://devguide.owasp.org/en/04-design/02-web-app-checklist/10-handle-errors-exceptions/) reinforces these principles with a focus on not disclosing internal details to end users, not disclosing sensitive user data to developers, and ensuring secure failure modes that don't weaken authentication or authorization.
...
- **Validate strictly and abort on errors**: Unexpected formats or values should be treated as errors. Don't continue in a partially verified state. For example, if a network call succeeds at the transport layer but fails validation at the application layer, processing must stop. If the validation fails, don't try to sanitize the data to make the validation succeed.
