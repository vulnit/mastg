...
2. Use one term for each concept. Do not change terminology or wording for stylistic variation.
...
7. Write short, complete sentences. Do not omit necessary words.
...
14. Do not use semicolons. Use hyphens to connect words that function as one unit.
...
3. Remove words that do not change the meaning.
...
7. Write Android versions as "Android X (API level YY)". Do not use codenames.
8. Spell out a term before its abbreviation on first use in body text. If it first appears in a heading, define it in the following text. Do not abbreviate a term used only once.
...
16. Do not duplicate content from another section of the guide. Link to the existing content instead.
17. Describe commercial tools and services factually. Do not use promotional language.
18. Use a numbered list when item order is essential. Use a bulleted list when item order is not essential.
19. Do not add end punctuation to list items that are not complete sentences unless they complete the introductory sentence. Use end punctuation for complete sentences.
...
23. For a generic technical term, prefer the spelling in Merriam-Webster's Collegiate Dictionary, 11th edition. If it is not available there, use the Microsoft Manual of Style, 4th edition, and then FOLDOC.
