...
- Determine whether the update check executes before access to protected functionality or backend services and can't be bypassed (for example, by checking the call graph or entry point context).
...
- For backend-gated flows, determine whether the app compares the installed version against a backend-supplied minimum version and blocks access when the current version doesn't meet the policy.
