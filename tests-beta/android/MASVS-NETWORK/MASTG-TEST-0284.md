...
- **Relying only on primary error code:** using [`getPrimaryError()`](https://developer.android.com/reference/android/net/http/SslError#getPrimaryError()) for decision-making, such as proceeding if the primary error isn't `SSL_UNTRUSTED`, which may overlook additional errors in the chain.
