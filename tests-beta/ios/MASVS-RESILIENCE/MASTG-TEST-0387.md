...
These APIs are commonly used for unrelated purposes (for example, networking, analytics, or generic checksums), so their mere presence doesn't confirm a storage integrity mechanism. Inspect each reported code location using @MASTG-TECH-0076 to determine whether the data is protected:
