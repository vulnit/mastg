...
The AndroidManifest is stored in a binary XML format and can't simply be extracted from the APK by unzipping it. To properly analyze the manifest, you first need to extract and decode it into a human-readable XML format.
...
jadx outputs the manifest in full to `out_dir/resources/AndroidManifest.xml`, including the `<uses-sdk>` element, which isn't included when using other tools like apktool.
...
If you're only interested in specific values of the manifest, you can use aapt2.
