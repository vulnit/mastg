...
This sample Kotlin code fetches the @MASTG-TOOL-0143 server <https://mitm-software.badssl.com/> using `HttpsURLConnection`, which isn't normally allowed because the certificate isn't trusted by the system. However, due to the Network Security Configuration that permits user-added CA certificates, the connection is allowed to proceed.
