...
It comes in a (limited) free version and paid version with additional features. HTTP Toolkit allows to setup its CA certificate and disabling SSL pinning via Frida and is also able to intercept HTTP requests from Flutter. In the free version it isn't possible to set an up-stream proxy to re-direct the HTTP traffic to a penetration testing tool like BurpSuite.
